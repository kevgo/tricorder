...
pub fn determine_fixes(config: &Config, detected_stacks: &DetectedStacks) -> Result<Runnables> {
    ...
    for detected_stack in detected_stacks {
        ...
        let Some(stack_config) = stack_config else {
            // this stack has no config --> skip it
            continue;
        };
        let Some(stack_fixes) = &stack_config.fix else {
            continue;
        };
        if let Some(override_fixes) = &stack_fixes.replace {
        ...
        if let Some(additional_fix) = &stack_fixes.add {
