...
pub fn determine_fixes(config: &Config, detected_stacks: &DetectedStacks) -> Result<Runnables> {
    ...
    for detected_stack in detected_stacks {
        ...
        if let Some(override_fixes) = stack_config
            .and_then(|stack_config| stack_config.fix.as_ref())
            .and_then(|fix| fix.replace.as_ref())
        {
        ...
        if let Some(additional_fix) = stack_config
            .and_then(|stack_config| stack_config.fix.as_ref())
            .and_then(|fix| fix.add.as_ref())
        {
